<script lang="ts">
	...
	const {
		...
		app_id: AppID;
		on_close_app: () => void;
		on_minimize_app: () => void;
		on_maximize_click: () => void;
	} = $props();
</script>

<div class="container" class:unfocused={apps.active !== app_id}>
	<button class="close-light" aria-label="Close {apps_config[app_id].title}" onclick={on_close_app}>
		<CloseIcon />
	</button>
	<button
		class="minimize-light"
		aria-label="Minimize {apps_config[app_id].title}"
		onclick={on_minimize_app}
	>
		<MinimizeSvg />
	</button>
	<button
		class="stretch-light"
		aria-label="Zoom {apps_config[app_id].title}"
